...
PetscReal Background_Conc=0;
PetscReal Inlet_Conc=0;
PetscInt zero_grad=1;
...
PetscReal k_ss=0.0, w_s=0.0, Nseg=0.0, sed_density=20000, U_Bulk=0.0, d50=1000;
...
PetscErrorCode Ucont_P_Binary_Input(UserCtx *user)
{
  ...
  */
  ...
  if(user->temp) {
    ...
    DMGlobalToLocalBegin(user->da, user->gTmprt_o, INSERT_VALUES, user->lTmprt_o);
    DMGlobalToLocalEnd(user->da, user->gTmprt_o, INSERT_VALUES, user->lTmprt_o);
  ...
  if(conv_diff)
  {
    ...
    bool fexist = file_exist(filen);
    if ((rstart_flg || ti != 0) && fexist)
    {
      PetscViewerBinaryOpen(PETSC_COMM_WORLD, filen, FILE_MODE_READ, &viewer);
      PetscPrintf(PETSC_COMM_WORLD, "Reading %s !\n", filen);
    ...
    else
    {
      ...
      PetscPrintf(PETSC_COMM_WORLD, "\nInitializing Concentration eqn ......... \n\n");
      Conv_Diff_IC(user);
    }
  ...
  */
  ...
  if(averaging && ti!=0) {      // Seokkoo Kang
    ...
    if (conv_diff)
    {
      ...
      PetscViewerDestroy(&viewer);
      sprintf(filen, "%s/sconc_%06d_%1d.dat.info",path, ti, user->_this);
      if(!rank)
        unlink(filen);
      ...
      PetscViewerDestroy(&viewer);
      sprintf(filen, "%s/sconc_cross_sum_%06d_%1d.dat.info",path, ti, user->_this);
      if(!rank)
        unlink(filen);
  ...
  if(user->temp) {
    sprintf(filen, "%s/tfield%06d_%1d.dat", path, ti, user->_this);
    write_output_file (user, user->gTmprt, filen);
    /*
      PetscViewerBinaryOpen(PETSC_COMM_WORLD, filen, FILE_MODE_WRITE, &viewer);
...
void write_data(UserCtx *user)
{
  ...
  PetscReal ***p, ***aj, ***nvert, ***rho, ***level, ***Conc;
      ...
      for (i=lxs; i<lxe; i++) {
        ...
        for(int m=0; m<nsave_points; m++) {
          if(i==save_point[m][0] && j==save_point[m][1] && k==save_point[m][2]) {
            ...
            {
              fprintf(f, "\n");
              fprintf(f, "\n\n**** Beginning to log time history: ti cent.x, cent.y, cent.z, ucat.x, ucat.y, ucat.z, p, U, V, W, vort.x, vort.y, vort.z");
              if (conv_diff)
                fprintf(f, " conc");
              fprintf(f, "\n");
            }
            fprintf(f, "%d %.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e", (int)ti, cent[k][j][i].x, cent[k][j][i].y, cent[k][j][i].z, ucat[k][j][i].x, ucat[k][j][i].y, ucat[k][j][i].z, p[k][j][i], U, V, W, vort_x, vort_y, vort_z);
            if (conv_diff)
              fprintf(f, " %.7e", Conc[k][j][i]);
            fprintf(f, "\n");
            fclose(f);
            ...
            f = fopen(filen, "a");
            fprintf(f, "\n");
            fprintf(f, "\n\n**** Beginning to log time history: ti cent.x, cent.y, cent.z, du/dx, du/dy, du/dz, dv/dx, dv/dy, dv/dz, dw/dx, dw/dy, dw/dz, dp/dx, dp/dy, dp/dz\n");
...
// Specify sources of concentration.  These are not sources in the traditional
// sense, instead the grid point values for concentration are set to the
// values given in this file before and after each iteration.
// The source are specified in a file named conc_source.dat.  It has the
// following format:
//
// i j k radius 'conc value'
// or
// x y z radius 'conc value'
//
// The way this code tells the difference between the first or the second
// is that the i j k in the first are integer and the x y z in the second
// are floating point.  That means that x y z must contain a period or an
// exponent value.
//
// The grid point identified by i j k or x y z is always set to the given
// concentration.  If other grid point around the identified (first) grid
// point are within radius, they also will set set to the
// concentration value.  WRO 2020-04-08

// Storage for the concentration source information.
bool conc_src_present = false;
bool conc_src_init = false;
std::vector < ConcEntry > conc_array;

void readConcSourcePoints()
{
  bool header = false;
  ...
  if (filename[filename.length()-1] != '/')
    filename += '/';
  filename += "conc_source.dat";

  std::ifstream fin(filename);
  if (fin.good())
  {
    PetscPrintf(PETSC_COMM_WORLD, "Reading conc_source.dat file.\n");

    conc_src_present = true;
    int lineNum = 0;
    std::string buff;
    while(!fin.eof())
    {
      unsigned int i, j, k;
      double x, y, z, radius, conc;
      ...
      if (buff.length() == 0 || buff[0] == '#')
        continue;
      // debug
      // PetscPrintf(PETSC_COMM_WORLD, "rank: %d  leng: %d  buff: %s\n",
      //                                  my_rank, buff.length(), buff.c_str());

      // one of the first three number must contain a . or an 'e' to know
      // that it is a floating point number and, therefore, a coordinate.
      bool ijk = true;
      size_t newpos = buff.find_first_of("0123456789+-.eE");
      if (newpos != std::string::npos) {
        newpos = buff.find_first_of(" \t", newpos+1);
        if (newpos != std::string::npos) {
          newpos = buff.find_first_of("0123456789+-.eE", newpos+1);
          if (newpos != std::string::npos) {
            newpos = buff.find_first_of(" \t", newpos+1);
            if (newpos != std::string::npos) {
              newpos = buff.find_first_of("0123456789+-.eE", newpos+1);
              if (newpos != std::string::npos) {
                newpos = buff.find_first_of(" \t", newpos+1);
                if (newpos != std::string::npos) {
                  newpos = buff.substr(0, newpos).find_first_of(".eE");
                  if (newpos != std::string::npos)
                    ijk = false;
      } } } } } }
      ...
      if (ijk)
      {
        int cnt = sscanf(buff.c_str(), "%u %u %u %lf %lf",
                                                   &i, &j, &k, &radius, &conc);
        if (cnt == 5)
        {
          // debug
          // printf("rank: %d  i: %d  j: %d  k: %d  radius: %f conc: %f\n",
          //                                   my_rank, i, j, k, radius, conc);
          conc_array.push_back(ConcEntry(i, j, k, radius, conc));
        }
        else
          error = true;
      }
      ...
      {
        int cnt = sscanf(buff.c_str(), "%lf %lf %lf %lf %lf",
                                                   &x, &y, &z, &radius, &conc);
        if (cnt == 5)
        {
          // debug
          // printf("rank: %d  x: %f  y: %f  z: %f  radius: %f conc: %f\n",
          //                                  my_rank, x, y, z, radius, conc);
          conc_array.push_back(ConcEntry(x, y, z, radius, conc));
        }
        else
          error = true;
      }
      if (error)
      {
        if (!header)
        {
          header = true;
          PetscPrintf(PETSC_COMM_WORLD,
                      "A record in conc_source.dat has an invalid format.\n");
        }
        PetscPrintf(PETSC_COMM_WORLD,
                      "line number: %d  data: %s\n", lineNum, buff.c_str());
      }
    }

    // debug
    // printf("rank: %d  conc_array.size: %d\n", my_rank, conc_array.size());
...
// Specify sources of momentum source.  The source are specified in a file
// named moment_source.dat.  It has the following format:
//
// i j k dir u v w
// or
// x y z dir u v w
...
// exponent value.
//
// dir is the direction of the surface normal: 0: in i, 1: in j, 2: in k:
// plus values direct normal in positive index direction, negative values are
// in the opposite direction.  WRO 2020-06-30

// Storage for the momentum source information.
bool moment_src_present = false;
bool moment_src_init = false;
std::vector < MomentEntry > moment_array;

void readMomentSourcePoints()
{
  bool header = false;
  ...
  if (filename[filename.length()-1] != '/')
    filename += '/';
  filename += "moment_source.dat";

  std::ifstream fin(filename);
  if (fin.good())
  {
    moment_src_present = true;
    int lineNum = 0;
    std::string buff;
    while(!fin.eof())
    {
      ...
      // one of the first three number must contain a . or an 'e' to know that
      // it is a floating point number and, therefore, a coordinate.
      bool ijk = true;
      size_t newpos = buff.find_first_of("0123456789+-.eE");
      if (newpos != std::string::npos) {
        newpos = buff.find_first_of(" \t", newpos+1);
      ...
      if (ijk)
      {
        int cnt = sscanf(buff.c_str(), "%u %u %u %d %lf %lf %lf",
                                                 &i, &j, &k, &dir, &u, &v, &w);
        if (cnt == 7)
        {
          moment_array.push_back(MomentEntry(i, j, k, dir, u, v, w));
        }
        else
          error = true;
      }
      ...
      {
        int cnt = sscanf(buff.c_str(), "%lf %lf %lf %d %lf %lf %lf",
                                                 &x, &y, &z, &dir, &u, &v, &w);
        if (cnt == 7)
        {
          moment_array.push_back(MomentEntry(x, y, z, dir, u, v, w));
        }
        else
          error = true;
      }
    ...
    // debug
    // printf("rank: %d  moment_array.size: %d\n", my_rank,moment_array.size());
...
/*   PetscMalloc(sizeof(IBMNodes), &ibm1); */
  ...
  PetscOptionsGetInt(PETSC_NULL, "-conv_diff", &conv_diff, PETSC_NULL);         // Ali
  PetscOptionsGetInt(PETSC_NULL, "-density_current", &density_current, PETSC_NULL);             // Ali
  PetscOptionsGetInt(PETSC_NULL, "-SuspendedParticles", &SuspendedParticles, PETSC_NULL);       // Ali
  PetscOptionsGetReal(PETSC_NULL, "-w_s", &w_s, PETSC_NULL);                    // Ali
  PetscOptionsGetReal(PETSC_NULL, "-background_conc", &Background_Conc, PETSC_NULL);                    // Ali
  PetscOptionsGetReal(PETSC_NULL, "-inlet_conc", &Inlet_Conc, PETSC_NULL);                      // Ali
  ...
  PetscOptionsGetInt(PETSC_NULL, "-save_inflow", &save_inflow, PETSC_NULL);             // Seokkoo Kang: save infow BC to files; should be used in conjunction with -pseudo 1
  ...
  // WRO for debugging
  PetscOptionsGetInt(PETSC_NULL, "-fluxin_output", &fluxin_output, PETSC_NULL);
...
//        PetscOptionsGetInt(PETSC_NULL, "-num_innergrid", &num_innergrid, PETSC_NULL); // xyang 3-18-2011
  ...
  */
  ...
  // Read convection source points
  if (conv_diff)
    readConcSourcePoints();

  // Read moment source points
  readMomentSourcePoints();
...
//      if(fabs(dpdz_set)<1.e-10) dpdz_set=PETSC_FALSE;                                 // deactivate xiaolei
  ...
  if (user->temp)
    VecDuplicate(user->lP, &user->lQ_scalar); // add (xiaolei)
...
*/
    ...
    if (ti==0) {
      ...
      if (user[bi].temp && (user[bi].buoyTest || user[bi].buoyMouthNose))
        Tmprt_IC(&(user[bi]));
...
/*     Save the old values (at ti) for later */
    ...
    for (bi=0; bi<block_number; bi++) {
      ...
      if (user->temp)
      {
        ...
        VecCopy(user[bi].gTmprt_o, user[bi].gTmprt_rm1);
        VecCopy(user[bi].gTmprt, user[bi].gTmprt_o);

        DMGlobalToLocalBegin(user[bi].da, user[bi].gTmprt_o, INSERT_VALUES, user[bi].lTmprt_o);
        DMGlobalToLocalEnd(user[bi].da, user[bi].gTmprt_o, INSERT_VALUES, user[bi].lTmprt_o);

        DMGlobalToLocalBegin(user[bi].da, user[bi].gTmprt_rm1, INSERT_VALUES, user[bi].lTmprt_rm1);
        DMGlobalToLocalEnd(user[bi].da, user[bi].gTmprt_rm1, INSERT_VALUES, user[bi].lTmprt_rm1);

      }

      if (conv_diff)
      {
        PetscPrintf(PETSC_COMM_WORLD, "save old concentration\n");
