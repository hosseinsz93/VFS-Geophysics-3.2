...
PetscErrorCode TECIOOut_rhs1(UserCtx *user, Vec Rhs)
{
  ...
  char filen[80];
  sprintf(filen, "rhs_%06d_%03d.plt", ti, rank);
...
void Pressure_Gradient(UserCtx *user, Vec dP)
{
  ...
  /* First we calculate the flux on cell surfaces. Stored on the upper integer
	...
	for (i=lxs; i<lxe; i++) {
		...
		if ((int)(nvert[k+1][j][i]+0.5)==1 || (int)(nvert[k+1][j][i+1]+0.5)==1 || (k==mz-2 && !k_periodic && !kk_periodic))
                  dpdz = (p[k][j][i  ] - p[k-1][j][i  ] + p[k][j][i+1] - p[k-1][j][i+1]) * 0.5;
		else if ((int)(nvert[k-1][j][i]+0.5)==1 || (int)(nvert[k-1][j][i+1]+0.5)==1 || (k==1 && !k_periodic && !kk_periodic))
...
PetscErrorCode FormFunction_SNES(SNES snes, Vec Ucont, Vec Rhs, void *ptr)
{
	...
	// begin add (xiaolei)
        ...
        if (Force_wm && ( imin_wm != 0 || imax_wm != 0 || jmin_wm != 0 || jmax_wm != 0 || (IB_wm && immersed)))
          VecAXPY(Rhs, 1, user->Force_wm);  // xyang 0521
        ...
        {
          // Cmpnts ***gftmprt;
          // DMDAVecGetArray(user->fda, user->gFTmprt, &gftmprt);
          // TECOutCmpnts("gftmprt", user, gftmprt);
          // DMDAVecRestoreArray(user->fda, user->gFTmprt, &gftmprt);

          VecAXPY(Rhs, 1, user->gFTmprt);
        }

        if (user->ctemp)
          VecAXPY(Rhs, 1, user->FCurrent);
