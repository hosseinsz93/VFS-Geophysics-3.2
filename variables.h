...
struct Cmpnts
{
    ...
    Cmpnts(Cmpnts const & c)                : x(c.x), y(c.y), z(c.z) { }
    ...
    double max() const
    {
      double maxc = (abs(x) > abs(y)) ? abs(x) : abs(y);
      maxc = (abs(z) > maxc) ? abs(z) : maxc;
      return maxc;
    }
};
...
typedef struct {
  ...
  //PetscReal *_x_bp, *_y_bp, *_z_bp;	// for rank 0
    ...
    PetscReal     d50, U_Bulk,sed_density;
  ...
  /* Variables for multi-nullspace case */
	...
	*/
	...
	// rotor_model
        ...
        // Turn on temperature calculations. (using tmprt.c)
        PetscInt temp;

        // Turn on temperature calculations. (using convection_diffusion.c)
        PetscInt ctemp;

        // Initital temperature condition. defaults to z.
        PetscReal temp_IC;

        // Buoyancy filename that contains the temperature BCs.
        PetscBool buoyFilenameSet;
        std::string buoyFilename;

        Vec gTmprt, lTmprt, gTmprt_o, lTmprt_o, gTmprt_rm1, lTmprt_rm1;//, K_Omega_rm1;

        Vec gFTmprt;            //buoyancy force
        Vec lFTmprt;
	...
	// Buoyancy driven flow
        ...
        // + calculate beta
        PetscReal buoyTempRef;
        PetscReal buoyBeta;
  
        // For -mouth_nose_buoy and -test_buoy,
        // the temp of hot or cold areas (degrees C) (e.g. surface temperature)
        PetscReal buoyTemp;
...
extern PetscInt les, wallfunction, rans, lowRe, slipbody, central, second_order, weno;
...
// Store the concentration source information.
extern bool conc_src_present;
extern bool conc_src_init;

class ConcEntry
{
  public:
    ConcEntry(unsigned ai, unsigned aj, unsigned ak, double aradius, double aconc)
    {
      dlete = false;
      type = 1;
      i = ai;
    ...
    ConcEntry(double ax, double ay, double az, double aradius, double aconc)
    {
      dlete = false;
      type = 2;
      i = j = k = 0;
    ...
    void dump(int rank, int idx)
    {
      printf("rank: %d  entry: %d", rank, idx);
...
extern std::vector < ConcEntry > conc_array;


// Store the momentum source information.
extern bool moment_src_present;
extern bool moment_src_init;

class MomentEntry
{
  public:
    MomentEntry(unsigned ai, unsigned aj, unsigned ak, int adir,
    ...
    MomentEntry(double ax, double ay, double az, int adir,
                double au, double av, double aw)
      : dlete(false)
      , type(2)
      , i(0)
    ...
    {
      printf("rank: %d  entry: %d", rank, idx);
      printf("  i,j,k,dir: %d %d %d %d", i, j, k, dir);
      printf("  x,y,z: %f %f %f", x, y, z);
      printf("  u,v,w: %f %f %f\n", u, v, w);
    }
    ...
    // private:
    bool dlete;
    int type;                           // type == 1: i,j,k specified
                                        //      == 2: x,y,z specified
    int dir;                            // Surface normal: 0: in i, 1: in j,
                                        // 2: in k, plus values direct normal
                                        // in positive index direction,
                                        // negative values in the opposite
                                        // direction.
    int i, j, k;
    double x, y, z;

    double u, v, w;                     // Source velocities
};
extern std::vector < MomentEntry > moment_array;
...
extern PetscInt sediment, input_ib_depth, conv_diff, SuspendedParticles, mobile_bed, projection_method, periodic_morpho, density_current, conv_diff, SuspendedParticles;
extern PetscReal Background_Conc, Inlet_Conc;
